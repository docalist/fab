...
//    }
    ...
    public function searchForm($error='')
    {
        ...
        (
            ...
            array('error'=>$error),             // Message d'erreur éventuel
     ...
     * configuration ou lance une recherche si une équation peut être construite
     * à partir des paramètres passés et affiche les notices obtenues en
     * utilisant le template indiqué dans la clé 'list' de la configuration.
     */
    public function actionSearch()
    {
        ...
        // Si aucun paramètre de recherche n'a été passé, il faut juste afficher le formulaire
        if (is_null($this->equation)) 
            return $this->searchForm();
        
        // Des paramètres ont été passés, mais tous sont vides et l'équation obtenue est vide
        if ($this->equation==='')
            return $this->searchForm("Vous n'avez indiqué aucun critère de recherche.");
        ...
        if ($selection->count == 0)
            return $this->searchForm("Aucune réponse. Equation : $this->equation");

        // Détermine le template à utiliser
        if (! $template=$this->getTemplate('list'))
            throw new Exception('Le template à utiliser n\'a pas été indiqué');
    ...
    public function actionSave()
    {
        ...
        $url=Routing::linkFor('show?ref='.$ref, true);
