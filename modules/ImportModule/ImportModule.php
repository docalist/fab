...
//            echo '1ère ouverture de la base en écriture<br />';
            ...
            echo '<p>Début de l\'import du fichier ', $fileName, ' le ', strftime('%x à %X'), '</p>';
...
//            echo '2e fermeture de la base<br />';
    ...
    public function actionUpload()
    {
        ...
        foreach($_FILES as $file) // todo: on ne devrait pas utiliser $_FILES, request devrait avoir les méthodes pour gérer les fichiers
        {
            ...
            $result=$this->uploadFile($file, $path, $callback);
...
//        echo 'upload done';
     ...
     * file était vide).
     */
    private function uploadFile($file, $path, $callback=null)
    {
        switch($file['error'])
        {
            case UPLOAD_ERR_OK:
            ...
            case UPLOAD_ERR_INI_SIZE:
                return sprintf("Impossible de charger le fichier '%s' : la taille du fichier dépasse la taille maximale autorisée par le serveur (upload_max_filesize=%s)", $file['name'], ini_get('upload_max_filesize'));
