...
//                <sortable start="0" end=")" />
        ...
        if (isset($def['alias'])) foreach($def['alias'] as $alias)
        {
            ...
            if (isset($def['index'][$name]))
                throw new Exception("Impossible de définir l'alias '$name' : ce nom existe déjà comme");
...
//************************************
    ...
    {
        // Crée la base xapian
        $this->xapianDatabase=new XapianWritableDatabase($path, Xapian::DB_CREATE);
        
        // Enregistre la structure de la base
        $this->xapianDatabase->set_metadata('structure', serialize($structure));
        $this->xapianDatabase->flush();

        $this->record=new XapianDatabaseRecord2($this, $this->fields);
