...
var Editor =
{
    ...
    loadStructure: function ()
    {
        ...
        for (var prop in this.db)
        {
            ...
            if (this.db[prop] instanceof Array)
            {
                ...
                // Initialise l'évènement onchange du select
                select.change(Editor.select);
                ...
                select.keydown(
                    function (e)
                    {
                        if (e.keyCode === 46)
                        { 
                            Editor.deleteOption(this, e.shiftKey === true);
                        }
                    }
                );

                // Crée un expando 'current' pour le select
                select.get(0).current = -1;
                
                var array = this.db[prop];
                
                // Pour chaque élément du tableau, on ajoute une option au select
                for (var i in array)
                {
                    ...
                    {
                        select.get(0).add(option, null); // standards compliant
                    }
                    catch (exception)
                    {
                        select.get(0).add(option); // IE only
                    } 
                }
                
                // Sélectionne le premier élément du select ou masque le right panel si le tableau est vide
                select.change();
            }
    ...
    select: function (e)
    {
        ...
        Editor.loadObject('#' + this.id, this.options[this.selectedIndex].data);
    },
            ...
            if (data instanceof Array)
            {
                ...
                {    
                    ...
                    for (var i in data)
                    {
                        ...
                        Editor.loadObject(propid, data[i], tr);
                        table.append(tr);
                    }
                    ...
                    jQuery('button.deletetr', table).click(
                        function ()
                        {
                            Editor.deleteTr(jQuery(this).parents('tr:first').get(0));
                        }
                    );
        ...
        for (var prop in what)
        {
            ...
            if (data instanceof Array)
            {
                ...
                var length = jQuery('tr:not(:first)', table).each(
                    function (i)
                    {
                        Editor.saveObject(propid, data[i], this);
                    }
                ).length;
    ...
    init: function (db)
    {
        ...
        jQuery('button.moveup').click(
            function ()
            {
                Editor.moveOption(jQuery('select', jQuery(this).parent().parent()).get(0), -1);
            }
        );
        jQuery('button.movedown').click(
            function ()
            {
                Editor.moveOption(jQuery('select', jQuery(this).parent().parent()).get(0), 1);
            }
        );
        jQuery('button.addoption').click(
            function ()
            {
                Editor.addOption(jQuery('select', jQuery(this).parent().parent()).get(0));
            }
        );
        jQuery('button.deleteoption').click(
            function ()
            {
                Editor.deleteOption(jQuery('select', jQuery(this).parent().parent()).get(0));
            }
        );
        jQuery('button.addtr').click(
            function ()
            {
                Editor.addTr(jQuery('table', jQuery(this).parent()).get(0));
            }
        );
        jQuery('button.deletetr').click(
            function ()
            {
                Editor.deleteTr(jQuery(this).parents('tr:first').get(0));
            }
    ...
    addOption: function (select)
    {
        ...
        if (typeof Editor.defaults[select.id] === 'undefined')
        {
            console.error('Impossible de trouver defaults[', select.id, ']');
            return;
        }
            
        var data = Editor.clone(Editor.defaults[select.id]);
...
*/        
    ...
    addTr: function (table)
    {
        ...
        Editor.saveObject('#' + id, data);
        
        //  et on ajoute un élément au tableau data[prop]
        if (typeof Editor.defaults[table.id] === 'undefined')
        {
            console.error('Impossible de trouver defaults[', table.id, ']');
            return;
        }
        
        data[prop].push(Editor.clone(Editor.defaults[table.id]));
        
        // Ensuite on demande un réaffichage de l'option
        Editor.loadObject('#' + id, data);
        jQuery('tr:last', table).hide().fadeIn(Editor.fxSpeed);
    ...
    deleteTr: function (tr)
    {
        ...
        if (! table) 
        {
            ...
            // sur les button.deletetr, du coup on est appellé deux fois, 
...
//        this.option.field.name = /*this.option.text=*/jQuery('#field_name').val();
        ...
        jQuery.ajax(
            {
                type: 'POST',
                url: this.saveUrl,
                data: jQuery.extend({structure: jQuery.toJSON(this.db)}, this.saveParams), /* clone(); */
                dataType: 'json',
                success: function (data)
                {
                    if (typeof(data) === 'string')
                    {
                        if (confirm("Aucune erreur détectée, la structure de la base a été enregistrée dans le fichier " + Editor.saveParams.template))
                        {
    ...
    /**
     * Sauvegarde la structure complète de la base
     */
    oldsave : function ()
    {
        // Stocke les modifications apportées au champ en cours
        if (this.option) 
        {
            this.saveField();
        }

        // Construit la structure à partir de ce qu'on a en mémoire
        var db = jQuery.extend({}, this.defaultDb);

        this.saveObject(db, 'db');
        jQuery('option', this.select).each(
            function (i)
            {
                db.field[i] = this.field;
            }
        );
        
        jQuery('#aliastable tr:not(:first)').each(
            function (i)
            {
                db.alias[i] = jQuery.extend({}, Editor.defaultAlias);

                //db.alias[i]=Editor.defaultAlias;
                console.log('alias neuf', db.alias[i]);
                Editor.saveObject(db.alias[i], 'alias', this);
            }
        );

        // Masque la zone d'erreurs si elle est affichée
        jQuery('#errors').hide();
        var url = this.saveParams.url;
        delete this.saveParams.url;

        jQuery.ajax(
            {
                type: 'POST',
                url: this.saveUrl,
                data: jQuery.extend({structure: jQuery.toJSON(db)}, this.saveParams), /* clone(); */
                dataType: 'json',
                success: function (data)
                {
                    if (typeof(data) === 'string')
                    {
                        if (confirm("Aucune erreur détectée, la structure de la base a été enregistrée dans le fichier " + Editor.saveParams.name))
                        {
                            window.location = data;
                        }
                    }
    ...
    load: function (data)
    {
        ...
        case 'string':
            jQuery.getJSON(
                ...
                {
                    Editor.init(json); 
                    jQuery('#DbEditor').fadeIn(Editor.fxSpeed);
                }
            );
            break;
        ...
        case 'object':
            ...
            break;
        default:
            alert('Impossible de déterminer le type de données passé à Editor.load()');
        }
    },
